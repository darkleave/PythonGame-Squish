...
class Level(State):
    """
    游戏关卡，它计算落下了多少个铅锤，移动精灵并执行其它与游戏逻辑相关的任务
    """
    ...
    def update(self, game):
        ...
        # 否则，如果铅锤已落到地上，那么就将其复位
        # 如果躲开了关卡内的所有铅锤，就让游戏切换到LevelCleared状态
    ...
    def display(self, screen):
        """
        在第一次显示（清屏后）显示状态，不同于firstDisplay,
        这个方法调用pygame.display.update并向它传递一个需要更新的矩形列表，
        这个列表是由self.sprites.draw 提供的
        :param screen:
        :return:
        """
        screen.fill(config.background_color)
...
class Paused(State):
    """
    简单的游戏暂停状态，用户可通过点击任何键盘键或单击鼠标来结束这种状态
    """
    finished = 0  # 用户结束暂停了吗?
    ...
    def handle(self, event):
        """
        这样来处理事件:将这项任务委托给State(它只处理退出事件),并对键盘和鼠标单击做出响应.
        如果用户按下了键盘建或者单击了鼠标，
    ...
    def update(self, game):
        """
        更新关卡。如果用户按下了键盘键或者单击了鼠标(即self.finished 为True),
    ...
    def first_display(self, screen):
        """
        在首次显示游戏暂停状态时调用，它绘制图像（如果指定了）并渲染文本
        :param screen:
        :return:
        """
        # 首先，通过使用背景色填充屏幕来清屏：
...
class LevelCleared(Paused):
    ...
    def __init__(self, number):
        self.number = number
        self.text = '''
        Level {} Cleared Click to start next level'''.format(self.number)
...
class GameOver(Paused):
    """
    指出游戏已结束的状态，紧跟在它后面的是表示游戏第一关的Level 状态
    """

    next_state = Level
    text = '''
    Game over Click to Restart,Esc to Quit'''
